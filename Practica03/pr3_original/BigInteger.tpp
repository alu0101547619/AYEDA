...
template <unsigned char Base>
BigNumber<Base>* BigInteger<Base>::add(const BigNumber<Base>& other) const override {
  const BigInteger<Base>* otherBigInteger = dynamic_cast<const BigInteger<Base>*>(&other);
  if (!otherBigInteger) {
    throw std::invalid_argument("Error: No se puede sumar un BigNumber incompatible.");
  }
  return new BigInteger<Base>(*this + *otherBigInteger);
}


template <unsigned char Base>
BigNumber<Base>& BigInteger<Base>::subtract(const BigNumber<Base>& other) const {
  const BigInteger<Base>* otherBigInteger = dynamic_cast<const BigInteger<Base>*>(&other);
  if (!otherBigInteger) {
...
template <unsigned char Base>
BigNumber<Base>& BigInteger<Base>::multiply(const BigNumber<Base>& other) const  {
  const BigInteger<Base>* otherBigInteger = dynamic_cast<const BigInteger<Base>*>(&other);
  if (!otherBigInteger) {
...
template <unsigned char Base>
BigNumber<Base>& BigInteger<Base>::divide(const BigNumber<Base>& other) const {
  const BigInteger<Base>* otherBigInteger = dynamic_cast<const BigInteger<Base>*>(&other);
  if (!otherBigInteger) {
...
template <unsigned char Base>
std::istream& BigInteger<Base>::read(std::istream& in) {
    std::string input;
    in >> input;

    if (input[0] == '-') {
        sign_ = true;
        input = input.substr(1); // Eliminamos el signo negativo
    } else {
        sign_ = false;
    }

    number_ = BigUnsigned<Base>(input.c_str());
    return in;
}
...
template <unsigned char Base>
BigUnsigned<Base> BigInteger<Base>::operator BigUnsigned<Base>() const {
  return number_;  // Devolvemos el valor sin signo de number_
}

template <unsigned char Base>
BigRational<Base> BigInteger<Base>::operator BigRational<Base>() const {
  return BigRational<Base>(number_, BigUnsigned<Base>(1));  // Numerador es el número y denominador es 1
}