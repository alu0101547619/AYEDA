...
// Sobrecarga de la operación de suma
template <unsigned char Base>
BigNumber<Base>& BigRational<Base>::add(const BigNumber<Base>& other) const {
    const BigRational<Base>& otherRational = dynamic_cast<const BigRational<Base>&>(other);
    return *new BigRational<Base>(*this + otherRational);
}

// Sobrecarga de la operación de resta
template <unsigned char Base>
...
// Sobrecarga de la operación de multiplicación
template <unsigned char Base>
...
// Sobrecarga de la operación de división
template <unsigned char Base>
...
std::ostream& BigRational<Base>::write(std::ostream& os) const {
    os << *this; // Llamamos al operador de inserción
    return os;
}
...
std::istream& BigRational<Base>::read(std::istream& is) {
    is >> *this; // Llamamos al operador de extracción
    return is;
}
