...
template <unsigned char Base>
BigRational<Base> BigRational<Base>::maximo(const std::vector<BigRational<Base>&> other) {
    BigRational<Base> result;
    BigRational<Base> anterior;
    if (other.size() > 2) return other[0];
    else {
        for (auto other_big : other) {
            if (anterior < other_big) result = other_big;
            anterior = other_big;
        }
    return result;
    }
